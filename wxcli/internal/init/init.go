...
type initWXCli struct {
	Directory      string `wxcli:"mode=arg,help=output directory"`
	Force          bool   `wxcli:"help=Do not check if output dir is empty"`
	SrcControlHost string `wxcli:"help=Repo domain or host"`
	Owner          string `wxcli:"help=Repo owner. Defaults to $USERNAME"`
...
func New() wxcli.WXCommand {
	...
	return wxcli.NewSub(in) //.Name("init")
}

func (in *initWXCli) Run() error {
	if !in.Force {
...
require github.com/wxio/wxcli v1.0.1
`,
	},
	{
		Path: "main.go",
...
`,
	...
	{
		Path: "internal/initwxcli/init.go",
		Tmpl: `package initwxcli
