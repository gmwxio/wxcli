...
func (n *subnode) parse(args []string) error {
...
func (n *subnode) addStructFields(group string, sv reflect.Value) error {
	if sv.Kind() != reflect.Struct {
		return fmt.Errorf("wxcli: %s should be a pointer to a struct (got %s)", sv.Type().Name(), sv.Kind())
	}
...
func (n *subnode) addStructField(group string, sf reflect.StructField, val reflect.Value) error {
...
func (n *subnode) addKVField(kv *kv, fName, help, mode, group string, val reflect.Value) error {
	...
	case "flag":
		...
		if e, ok := kv.take("env"); ok || n.useEnv {
			...
			_, set := n.envNames[e]
			if set && explicit {
				return fmt.Errorf("env name '%s' already in use", e)
			}
			if !set {
				n.envNames[e] = true
				i.envName = e
		...
		if short, ok := kv.take("short"); ok {
			if len(short) != 1 {
				return fmt.Errorf("short name '%s' on flag '%s' must be a single character", short, name)
			}
			if n.flagNames[short] {
				return fmt.Errorf("short name '%s' on flag '%s' already exists", short, name)
			}
	...
	case "arg":
		...
		if i.slice {
			if m, ok := kv.take("min"); ok {
				...
				if err != nil {
					return fmt.Errorf("min not an integer")
				}
				i.min = min
			}
			if m, ok := kv.take("max"); ok {
				...
				if err != nil {
					return fmt.Errorf("max not an integer")
				}
				i.max = max
			}
		}
		//validations
		if group != "" {
			return fmt.Errorf("args cannot be placed into a group")
		}
		if len(n.cmds) > 0 {
			return fmt.Errorf("args and commands cannot be used together")
		}
		for _, item := range n.args {
			if item.slice {
				return fmt.Errorf("cannot come after arg list '%s'", item.name)
			}
		}
...
func (n *subnode) setCmdName(val reflect.Value) error {
	if n.cmdname != nil {
		return fmt.Errorf("cmdname set twice")
	} else if val.Type().Kind() != reflect.String {
		return fmt.Errorf("cmdname type must be string")
	} else if !val.CanAddr() {
		return fmt.Errorf("cannot address cmdname string")
	}
	n.cmdname = val.Addr().Interface().(*string)
	return nil
}

func (n *subnode) addInlineCmd(name, help string, val reflect.Value) error {
	vt := val.Type()
...
// func (n *subnode) addInternalFlags() error {
...
// func (n *subnode) addFlagsets() error {
